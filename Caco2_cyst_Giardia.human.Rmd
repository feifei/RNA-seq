---
title: "edgeR for DE analysis of Lauras Caco-2 cells interacting with Giardia cyst cells, Human part"
author: "Feifei Xu"
date: "`r Sys.Date()`"
output: 
  html_document:
...
# Background
This report provides the analysis of RNA-Seq data from Caco-2 cells interacting with *Giardia* cyst cells, only the Caco2 cell (human) part. There is basically no *Giardia* reads since cysts were washed away, so no *Giardia* parts of analysis done.

*Giardia* cells was growing in TYDK media, and high concentration of bile was added to the media to induce encystation. After 7h into encystation (no cysts yet), those *Giardia* cells were removed from TYDK media, and put into DMEM media (which is a human caco-2 cell media, and apparently *Giardia* cells do not grow happily in this media). *Giardia* cysts were add to human caco-2 cells sitting in DMEM media for 1.5h, 3.5h and 4.5h, and we want to see what happens in human cells during the interaction with cysts. Control is the human cell without cysts added. 3 replicates were obtained for each state.
...
cell <- "cyst"
directory <- "data/"
meta_file <- "data/mapping_stats.cyst.txt"
```

## Data summary

```{r}
d <- read.table(meta_file, sep = "\t", header=T)

condition <- c()
batch <- c()
hour <- c()
for (x in sub("^TL-2800-Cac", "", d$prefix)) {
  #print(i)
  if (grepl("^ctrl", x) ) {
    condition <- c(condition, "control")
    hour <- c(hour, 0)
  } else {
    condition <- c(condition, "infection")
    num <- substring(x, 1, 1)
    if (num %in% c("3")) {
      hour <- c(hour, num)
    }else {
      hour <- c(hour, paste0(num, '.5'))
    }
  }
  if (nchar(x) == 1 | x == 'ctrl') {
    batch <- c(batch,  1)
  } else {
    last_char <- substr(x, nchar(x) - 1 + 1, nchar(x))
    batch <- c(batch, strtoi(last_char))
  }
}

count_files <- paste(d$prefix, ".", name, ".counts", sep = "")
d$count_files <- count_files

group <- factor(paste(condition, hour, sep = "_"))
d <- cbind(d, group = group, batch=batch, hour=hour, condition=condition)

d <- d[with(d, order(hour, group)), ]
group <- d$group
...
barplot(t(as.matrix(cbind(d$n_human_reads, d$n_giardia_reads))), 
...
```{r}
y <- readDGE(d$count_files, path = "counts_cyst", group = d$group, header = FALSE)
colnames(y$counts) <- d$group
...
names(counts_per_sample) <- d$group
cols <- as.numeric(d$group) + 2
...
for (contrast in names(qlf_d) ) {
  ...
  all.genes <- topTags(qlf, n=Inf, sort.by="logFC") # Return a list containing a data.frame tabl
