---
title: "edgeR for DE analysis of Caco-2 cells interacting with trophozoite Giardia cells, Giardia part"
author: "Feifei Xu"
date: "`r Sys.Date()`"
output: 
  html_document:
...
annotation_tab <- "data/wb_annotation.pseudo0.tab"
...
d <- d[-c(2,7,12), ] # Remove Human controls
...
## Data filtering
Only genes with reasonable expression level should be included in the analysis. Keep genes with at least 2 counts per million in at least 2 samples (3 replicates).
...
```
## Library sizes
Plot number of reads matched per sample, and total for each gene across all samples. Library sizes varies from 1 to 5 millions.
...
The samples were collected in 3 different batches (replicates). If we look at the sample clustering after removing this batch effect, samples from the same condition and time cluster more togther. Control is well separated from the rest, but the three infection time points do not seperate well, specially 3h and 4.5h. At least the batch adjustment seem to make sense. So we are continuing the DE analysis with batch adjusted data.
...
colnames(design)[4:6] <- levels(group)[2:4]
```
...
write.csv(cpm, file=paste(directory, name, ".", cell, ".cpm", tag, ".csv", sep=""))

```


```{r}
# Save the matrix for combined analysis
...
```{r}
# Export genes which are specific to the different time point
logFC_mat_interested <- logFC_mat[, c("Infection_1.5vs0", 
                                      "Infection_3vs0",
                                      "Infection_4.5vs0")]

DT_mat_interested <- data.frame(DT_mat_interested)
DT_1.5vs_0_specific <- DT_mat_interested[abs(DT_mat_interested$Infection_1.5vs0) >= 1 & abs(DT_mat_interested$Infection_3vs0) < 1 & abs(DT_mat_interested$Infection_4.5vs0) < 1,]
annotation_1.5vs0 <- sub_annotation[, -c(2)][sub_annotation$Geneid %in% rownames(DT_1.5vs_0_specific), ]
logFC_mat_1.5vs0 <- logFC_mat_interested[rownames(logFC_mat_interested) %in% rownames(DT_1.5vs_0_specific), ]
logFC_mat_1.5vs0$Geneid <- rownames(logFC_mat_1.5vs0)
combined_1.5vs0 <- merge(annotation_1.5vs0, logFC_mat_1.5vs0, by.x = "Geneid", by.y = "Geneid")
write.csv(combined_1.5vs0, file=paste(directory, name, ".", cell, ".1.5vs0", ".csv", sep=""), row.names = FALSE)

DT_3vs_0_specific <- DT_mat_interested[abs(DT_mat_interested$Infection_3vs0) >= 1 & abs(DT_mat_interested$Infection_1.5vs0) < 1 & abs(DT_mat_interested$Infection_4.5vs0) < 1,] 
annotation_3vs0 <- sub_annotation[, -c(2)][sub_annotation$Geneid %in% rownames(DT_3vs_0_specific), ]
logFC_mat_3vs0 <- logFC_mat_interested[rownames(logFC_mat_interested) %in% rownames(DT_3vs_0_specific), ]
logFC_mat_3vs0$Geneid <- rownames(logFC_mat_3vs0)
combined_3vs0 <- merge(annotation_3vs0, logFC_mat_3vs0, by.x = "Geneid", by.y = "Geneid")
write.csv(combined_3vs0, file=paste(directory, name, ".", cell, ".3vs0", ".csv", sep=""), row.names = FALSE)

DT_4.5vs_0_specific <- DT_mat_interested[abs(DT_mat_interested$Infection_4.5vs0) >= 1 & abs(DT_mat_interested$Infection_1.5vs0) < 1 & abs(DT_mat_interested$Infection_3vs0) < 1,] 
annotation_4.5vs0 <- sub_annotation[, -c(2)][sub_annotation$Geneid %in% rownames(DT_4.5vs_0_specific), ]
logFC_mat_4.5vs0 <- logFC_mat_interested[rownames(logFC_mat_interested) %in% rownames(DT_4.5vs_0_specific), ]
logFC_mat_4.5vs0$Geneid <- rownames(logFC_mat_4.5vs0)
combined_4.5vs0 <- merge(annotation_4.5vs0, logFC_mat_4.5vs0, by.x = "Geneid", by.y = "Geneid")
write.csv(combined_4.5vs0, file=paste(directory, name, ".", cell, ".4.5vs0", ".csv", sep=""), row.names = FALSE)
```
...
Venn Diagram of the DE genes with abs(logFC) >= 1.

```{r}
# Take columns interested
logFC_mat_interested <- logFC_mat[, c("Infection_1.5vs0", 
                                      "Infection_3vs0",
                                      "Infection_4.5vs0")]


# Transfer logFC values to 1, -1, 0 for Venn Diagram
logFC_mat_interested[logFC_mat_interested >= 1] <- 1
logFC_mat_interested[logFC_mat_interested <= -1] <- -1
logFC_mat_interested[logFC_mat_interested < 1  & logFC_mat_interested > -1] <- 0

vennDiagram(logFC_mat_interested, cex = c(1, 1, 0.8), main = "Common/unique DE genes with abs(logFC)>=1")
vennDiagram(logFC_mat_interested, cex = c(1, 1, 0.8), include = c("up", "down"), counts.col = c("red", 
    "blue"), main = "Common/uniquly regulated genes with abs(logFC)>=1")
