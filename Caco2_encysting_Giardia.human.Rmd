---
title: "edgeR for DE analysis of Lauras Caco-2 cells interacting with encysting Giardia cells, Human part"
author: "Feifei Xu"
date: "`r Sys.Date()`"
output: 
  html_document:
...
# Background
This report provides the analysis of RNA-Seq data from Caco-2 cells interacting with encystating *Giardia* cells, only the Caco2 cell (human) part. *Giardia* analysis is done in a separate file.
...
RNAs from *Giardia* control (171120B2, 171121C2) were taken after sitting in DMEM for 10m, All the Bs were done on the first day, Cs on the second, and Ds on the third day.
...
d <- d[-c(2,6), ] # Remove 171120B2 and 171121C2 which are Giairdia controls
...
human_encysting_logFC_mat <- logFC_mat
human_encysting_DT_mat <- DT_mat
save(human_encysting_logFC_mat, human_encysting_DT_mat, file="data/human.encysting.logFC.rda")
```
